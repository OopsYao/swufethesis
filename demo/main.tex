...
\usepackage{booktabs}
\usepackage{hyperref}
\usepackage{mwe} % 这个宏包提供了下面使用的示例图片，可以根据需要删除

% 使用BibLaTeX生成参考文献，这里引入参考文献的数据
\addbibresource{refs.bib}

\swufesetup{
  % ****************************
  % 注意
  % 1. \swufesetup命令内不要有空行
  % 2. 不需要的配置可以删除
  % ****************************
  %
  % 输出格式，可选值为print与check，分别表示打印版与检测版。默认为print。
  output = {print},
  %
  % 成文日期，默认为当天
  date = {2021-01-04},
  %
  % 标题，本科生只用中文标题
  title = {西南财经大学学位论文\LaTeX 模板示例文档 v\swufeversion},
  title* = {An Introduction to \LaTeX{} Thesis Template for Southwestern University of Finance and Economics
      v\swufeversion},
  %
  % 作者
  author = {韭菜盒子想毕业},
  ...
  supervisor = {XX教授},
  %
  % 学位，可选值为bachelor、master、doctor，分别对应本科、硕士和博士
  degree = {bachelor},
  %
  % 本科生届数，默认根据date选项进行设定
  index = {2017},
  %
  % 本科生学院
  school = {经济信息工程学院},
  %
  % 研究生学位类型，可选值academic、professional、equivalent，分别对应学术、专业型和同等学力，默认academic
...
\begin{document}
% \frontmatter 表示正文前面的内容，如摘要目录等。
% 从功能上讲，\frontmatter 标志着（正文前置内容）页码计数的开始，对于本科生来说有页码的摘要目录都是强制重新编码，\frontmatter 前后的位置没有影响（甚至不要都可以）；
% 而对研究生来说，如果不想将封面或者学术声明的页数也计入，就将 \frontmatter 放在它们的后面。
\frontmatter
...
\begin{figure}[htb]
  ...
  \includegraphics[width=0.4\textwidth]{example-image-b.pdf}
  \caption{图片示例}
\end{figure}
