...
可选项有 |print|、|check| 以及 |anon|，分别表示打印版、检测版和匿名版，默认为 |print|。
打印版会输出封面、承诺页、封底等页面，且保证章节页面都是右开的，从而
会产生一些必要的对打印友好的空白页；
检测版将去除封面、承诺页和封底；
匿名版适用于研究生论文，与打印版相同，但会隐藏姓名、学号和导师的信息。
如果致谢里有相关信息，可以使用 \cs{swufeanonmask} \oarg{alter} \marg{text} 来条件化地隐藏信息，具体见其说明。
\begin{latex}
  \swufesetup{
    output = print
  }
